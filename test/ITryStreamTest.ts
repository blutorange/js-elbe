...
[InplaceStreamFactory, TypesafeStreamFactory].forEach((factory, i) => {
    ...
    class ITryStreamTest extends StreamTest {
        ...
        public orElse() {
            this.stream([], s => s.try(trial).orElse(-1)).to.deep.equal([]);
        ...
        public orTry() {
            this.stream([], s => s.try(trial).orTry(e => 9)).to.deep.equal([]);
        ...
        public include() {
            this.stream([], s => s.try(trial).include(i => i < 3).orElse(-1)).to.deep.equal([]);
            this.stream([1,2,,4], s => s.try(trial).include(i => i < 3).orElse(-1)).to.deep.equal([1,2,-1,-1]);
        }

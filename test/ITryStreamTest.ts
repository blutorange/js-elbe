...
import { StreamTest, load } from "./utils";
...
load(({ InplaceStreamFactory, TryFactory, TypesafeStreamFactory }) => {
    [InplaceStreamFactory, TypesafeStreamFactory].forEach((factory, i) => {
        ...
        class ITryStreamTest extends StreamTest {
            ...
            public orTry() {
                this.stream([], s => s.try(trial).orTry(e => {throw new Error})).to.deep.equal([]);

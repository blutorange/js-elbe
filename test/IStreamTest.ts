...
import { Expect, load, setOf, toObj, StreamTest} from "./utils";  

export const hack: any[] = [];
load(({ InplaceStreamFactory, TypesafeStreamFactory, consumeFirst }) => {
    [InplaceStreamFactory, TypesafeStreamFactory].forEach((factory, i) => {
        ...
        class IStreamTest extends StreamTest {
            ...
            public cycle() {
                ...
                this.stream(gen(), s => s.cycle().limit(10)).to.deep.equal([0, 1, 2, 0, 1, 2, 0, 1, 2, 0]);
            ...
            @test("should consume only the first item")
            public consumeFirst() {
                const it1 = [1,2,3];
                const sink1: number[] = [];
                const after1 = consumeFirst(it1, sink1);
                expect(sink1).to.deep.equal([1]);
                expect(Array.from(after1)).to.deep.equal([2,3]);

                const it2 = "foo";
                const sink2: string[] = [];
                const after2 = consumeFirst(it2, char => sink2.push(char));
                expect(sink2).to.deep.equal(["f"]);
                expect(Array.from(after2)).to.deep.equal(["o", "o"]);
            }
            ...
            public consume() {
                ...
                const custom = {
                    [Symbol.iterator](): Iterator<number> {
                        let i = 0;
                        const x = {
                            next(arg: any): IteratorResult<number> {
                                i += 1;
                                return {
                                    done: i > 4,
                                    value: i,
                ...
                sink = [];const s2 = s(custom).consume(add, 0, 2);
                expect(sink).to.deep.equal([1,2]);
                expect(s2.toArray()).to.deep.equal([3,4]);
                ...
                this.terminal("foo", s => s.splice(0, 2)).to.deep.equal(["f", "o"]);
